const covid19ImpactEstimator = (data) => {
  ...
  // resolve to days
  if (data.periodType === 'days') period = 2 ** Math.trunc(data.timeToElapse / 3);
  else if (data.periodType === 'weeks') period = 2 ** Math.trunc((data.timeToElapse * 7) / 3);
  else if (data.periodType === 'months') period = 2 ** Math.trunc((data.timeToElapse * 30) / 3);
  ...
  severeImpact.hospitalBedsByRequestedTime = Math.trunc(bedAvailability - siscbrt);
